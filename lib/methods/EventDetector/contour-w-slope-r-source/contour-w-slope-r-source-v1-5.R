...
EventDetectoR<-function(soundFile=NULL,spectrogram=NULL,dataMini,ParamArgs){
  ...
  for(i in 1:length(cont)){
    ...
    if(size[i]>=pixThresh){ #& size[i]<1000
    imgSub<-as.cimg(image1[min(cont[[i]]$x):max(cont[[i]]$x),min(cont[[i]]$y):max(cont[[i]]$y),1,1])
    lines=hough_line(imgSub,data.frame = TRUE)
    
    Bestline<-lines[which.max(lines$score),]
    
    Bestline<- cbind(Bestline,(-(cos(Bestline$theta)/sin(Bestline$theta))),Bestline$rho/sin(Bestline$theta))
    Bestline[which(is.infinite(Bestline[,4])&Bestline[,4]<0),4]<-min(c(-2.76824e+18,min(Bestline[-which(is.infinite(Bestline[,4])),4])))
    Bestline[which(is.infinite(Bestline[,4])&Bestline[,4]>0),4]<-max(c(2.76824e+18,max(Bestline[-which(is.infinite(Bestline[,4])),4])))
    Bestline[which(is.infinite(Bestline[,5])&Bestline[,5]<0),5]<-min(c(-2.76824e+18,min(Bestline[-which(is.infinite(Bestline[,5])),5])))
    Bestline[which(is.infinite(Bestline[,5])&Bestline[,5]>0),5]<-max(c(2.76824e+18,max(Bestline[-which(is.infinite(Bestline[,5])),5])))
