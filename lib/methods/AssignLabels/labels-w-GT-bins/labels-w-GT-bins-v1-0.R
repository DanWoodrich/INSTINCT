...
args<-"C:/Apps/INSTINCT/Cache/f1f81de75ed5/ C:/Apps/INSTINCT/Cache/f1f81de75ed5/8237dc/ C:/Apps/INSTINCT/Cache/f1f81de75ed5/8a148e/ C:/Apps/INSTINCT/Cache/f1f81de75ed5/8a148e/037505  0.25 labels-w-GT-bins-v1-0"
...
routine = mean #median... etc? 


GTdata$probs<-0

#should do this in parallel, it takes a while
for(n in 1:nrow(GTdata)){
  print(n)
  GTrow=GTdata[n,]
  
  GTdata[n,"probs"]<-routine(outDataAll[which(outDataAll$StartFile==GTrow[,"StartFile"]&outDataAll$StartTime<GTrow[,"EndTime"]&outDataAll$StartTime>GTrow[,"StartTime"]),"probs"])
  
}

ggplot(GTdata, aes(x=meanprobs, fill=label)) +
