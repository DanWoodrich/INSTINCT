...
if(stage!="apply"){
  ...
  balance_tpfp<-args[8]
  cv_it<-as.integer(args[9])
  cv_split<-as.numeric(args[10])
  fp_multiple<-as.numeric(args[11])
  mtry<-as.integer(args[12])
  ntree<-as.integer(args[13])
  
  dataAL<-read.csv(dataPath_AL)
  dataFE<-read.csv(dataPath_FE)
  
  samecols<-c("StartTime","EndTime","LowFreq","HighFreq","StartFile","EndFile")
  data<-merge(dataFE,dataAL,by=samecols,all.x=TRUE)
  
  #remove any NA values
  if(any(is.na(data$label))){
    data<-data[-which(is.na(data$label)),]
  }

}else{
  data<-read.csv(dataPath_FE)
}
