...
probsAll<-foreach(i=1:cv_it,.packages = c("randomForest","doParallel")) %dopar% {
  ...
  data.rf<-randomForest(formula=label ~ . -Selection -FGID,data=dataTrain,mtry=mtry,ntree=ntree) 
  ...
  if(stage=="train"){
    ...
    probs<-data.frame(dataTest$Selection,probs[,2])
...
if(stage=="apply"){
  ...
  if(!any("FGID" %in% colnames(dataIn))){
    dataIn$FGID<-"" #ugly fix to make the model no break 
  }
  
  dataIn$Selection<-1:nrow(dataIn) #model will break if doesn't have this
