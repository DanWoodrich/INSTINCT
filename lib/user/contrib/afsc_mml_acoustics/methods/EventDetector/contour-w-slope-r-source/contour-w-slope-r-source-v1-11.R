...
EventDetectoR<-function(soundFile=NULL,spectrogram=NULL,dataMini,ParamArgs){
  ...
  #step=3000
  #len<-round(length(spectrogram$t)/step)
  #for(i in 0:(len-1)){
  #  extent<-((step*i)+1):(step*(i+1))
  #  tempogram<-spectrogram
  #  tempogram$S<-spectrogram$S[,extent]
  #  tempogram$t<-spectrogram$t[extent]
  ...
  for(i in 1:length(cont)){
    ...
    if(size[i]>=pixThresh){ #& size[i]<750
      ...
      #print(paste(i,size[i],min(cont[[i]]$x)))
      ...
      imgSub<-threshold(imgSub,ImgThresh2) #50% #fix this in v11
